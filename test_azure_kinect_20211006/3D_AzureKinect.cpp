...
KINECTDLL_API int nkinectdll = 0;
...
void pcl::KinectAzureDKGrabber::share_IMG(cv::Mat& share_img){
	std::unique_lock<std::mutex> lock(mutex);
	share_img = colorFrame;
	lock.unlock();
}
...
pcl::PointCloud<pcl::PointXYZ>::Ptr pcl::KinectAzureDKGrabber::convertDepthToPointXYZ() {
	...
	transformed_depth_image = k4a::image::create(K4A_IMAGE_FORMAT_DEPTH16,
		...
		color_image_width_pixels * (int)sizeof(std::uint16_t));
	k4a::image point_cloud_image = NULL;
	point_cloud_image = k4a::image::create(K4A_IMAGE_FORMAT_CUSTOM,
		color_image_width_pixels,
		color_image_height_pixels,
		color_image_width_pixels * 3 * (int)sizeof(std::int16_t));
	...
	std::int16_t* point_cloud_image_data = (std::int16_t*)(void*)point_cloud_image.get_buffer();
...
pcl::PointCloud<pcl::PointXYZI>::Ptr pcl::KinectAzureDKGrabber::convertInfraredDepthToPointXYZI() {
	...
	cloud->points.resize(cloud->height * cloud->width);
	std::int16_t* point_cloud_image_data = (std::int16_t*)(void*)point_cloud_image.get_buffer();
	std::int16_t* transformed_infrared_image_data = (std::int16_t*)(void*)transformed_infrared_image.get_buffer();
...
pcl::PointCloud<pcl::PointXYZRGB>::Ptr pcl::KinectAzureDKGrabber::convertRGBDepthToPointXYZRGB() {
	...
	cloud->points.resize(cloud->height * cloud->width);
	std::int16_t* point_cloud_image_data = (std::int16_t*)(void*)point_cloud_image.get_buffer();
	std:: uint8_t* color_image_data = colorImage.get_buffer();
#ifdef VTK_VISUALIZATION
	Eigen::Matrix3f m;
	m = Eigen::AngleAxisf(-M_PI, Eigen::Vector3f::UnitZ());
#endif
	for (int i = 0; i < width * height; ++i) {
		...
		std::uint8_t alpha = color_image_data[4 * i + 3];
...
int pcl::KinectAzureDKGrabber::Kinect_mainTheardCallBack(void* Kinectparm)
{
	KinectAzureDKGrabber* thisclassKinect = (KinectAzureDKGrabber*)Kinectparm;
